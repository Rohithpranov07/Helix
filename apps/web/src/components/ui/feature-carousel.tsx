...
interface CardProps {
  ...
  description: string
  bgClass?: string
  /** Renders title/description/step-label text for a light card background instead of the default dark-on-dark theme. */
  light?: boolean
}
...
// HELIX self-healing loop — the unifying thesis: divergence detected and corrected.
const steps: readonly Step[] = [
  {
    ...
    description:
      "The genome pairs intent to code while the immune scanner watches every module — drift, vulnerabilities and entropy are flagged the moment they appear.",
  },
  {
    id: "2",
    name: "Diagnose",
  ...
  },
  {
    id: "3",
    name: "Heal",
    title: "Heal autonomously",
    description:
      "A patch is synthesized, validated and applied without a human in the loop — the system repairs itself and mints an antibody so it never recurs.",
  },
  {
    id: "4",
    name: "Prove",
    title: "Prove before promotion",
    description:
      "Every change runs inside the Shadow twin. Nothing reaches production without a behaviour-equivalence proof — the Shadow invariant.",
  },
] as const
...
type MotionStepImageProps = React.ComponentProps<typeof MotionStepImage>
...
const AnimatedStepImage = ({
  ...
  // motion + exactOptionalPropertyTypes friction on style.x — assert through unknown
  const motionProps = {
    ...props,
    ...presetConfig,
    transition: { ...presetConfig.transition, delay },
    onAnimationComplete,
  } as unknown as MotionStepImageProps
...
function FeatureCard({
  ...
  return (
    ...
    >
      <div
        className={clsx(
          "group relative w-full overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-b from-neutral-950/90 to-neutral-900/90 transition duration-300",
        ...
        <div className="m-10 min-h-[450px] w-full">
          <AnimatePresence mode="wait">
            ...
            >
              ...
              >
                <p className={cn("text-sm leading-5 sm:text-base sm:leading-5", light ? "text-slate-600" : "text-neutral-300")}>
                ...
                <span className="flex items-center gap-2 text-sm font-medium">
                  <motion.span
                    ...
                    className={cn(
                      ...
                      isCurrent && "bg-emerald-400/20 text-emerald-300",
                  ...
                  <motion.span
                    ...
                    className={clsx(
                      ...
                      isCompleted && (light ? "text-slate-500" : "text-muted-foreground"),
                      isCurrent && (light ? "text-emerald-600" : "text-emerald-400"),
                      isFuture && (light ? "text-slate-400" : "text-neutral-500")
                    )}
                  >
                    {step.name}
                  </motion.span>
...
export const Component = ({
          ...
          return (
            ...
            >
              <AnimatedStepImage
                ...
                className={clsx(
                  step4imgClass,
                  "pointer-events-none top-[50%] w-[90%] overflow-hidden rounded-2xl border border-white/10 md:left-[35px] md:top-[30%] md:w-full"
                )}
  ...
  return (
    <FeatureCard {...props} light={light} step={step}>
      {renderStepContent()}
      <motion.div
        initial={{ opacity: 0 }}
        animate={{ opacity: 1 }}
      ...
      >
        <Steps current={step} onChange={() => {}} steps={steps} light={light} />
      </motion.div>
      <motion.div
