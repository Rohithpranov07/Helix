          ...
          {blocksDesign.map((component, index) => {
            return (
              <TimelineContent
                ...
                target="_blank"
                href={component?.url}
                className="transition-all aspect-video rounded-lg  backdrop-blur-sm overflow-hidden block relative"
                rel="noreferrer"
              >
                ...
                >
                  <h1 className="2xl:text-xl xl:text-xl md:text-lg text-sm font-medium leading-[140%] capitalize text-white">
                    {component.name} {component.blocks && (<span className="font-semibold">({component.blocks})</span>)}
                  </h1>
                </div>
              </TimelineContent>
            );
