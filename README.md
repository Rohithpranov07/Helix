...
| 2. Scan | Immune System scans ShopLite — finds SQLi, XSS, authBypass, secretLeak, missingRLS |
...
|-------|---------|-------------|
| `SQLi` | `/api/products/search` | Raw query string in SQL |
| `XSS` | `/api/reviews` | Unsanitised review content reflected |
| `authBypass` | `/api/orders/:id` | Missing ownership check |
| `secretLeak` | `/api/debug` | Env vars exposed in response |
| `missingRLS` | `profiles` table | Supabase RLS not enabled |
